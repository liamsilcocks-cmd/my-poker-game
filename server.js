...
function evaluateHand(cards) {
    ...
    // Scoring: 700=Quads, 600=FullHouse, 300=Trips, 200=2Pair, 100=Pair
...
setInterval(() => {
    ...
    if (blindTimer > 0) blindTimer--;
        ...
        debug(`Blinds Escalated to \${SB}/\${BB}`);
    }

    if (!['SHOWDOWN', 'DEALER_DRAW', 'LOBBY'].includes(gameStage)) {
        if (turnTimer > 0) turnTimer--;
        else {
            debug(`Timeout: Auto-folding \${players[playerOrder[turnIndex]].name}`);
...
function startNewHand() {
    ...
    debug(`Hand Started. Pot: \${pot}. Dealer: \${players[playerOrder[dealerIndex]].name}`);
    broadcast();
}
...
function handleAction(id, type, amount = 0) {
    ...
    let p = players[id];
    debug(`Action: \${p.name} -> \${type} \${amount || ''}`);
...
function nextStep() {
    ...
    if (roundOver) {
        debug(`Betting Round Complete. Moving from \${gameStage}`);
...
function showdown() {
    gameStage = 'SHOWDOWN';
    debug("Initiating Showdown Evaluator...");
    let winners = [];
    let bestScore = -1;
    ...
    playerOrder.forEach(id => {
        if (players[id].status === 'ACTIVE') {
            ...
            debug(`Eval: \${players[id].name} scored \${score}`);
    ...
    winners.forEach(id => {
        ...
        debug(`Awarding \${winAmt} to \${players[id].name}`);
    });
...
app.get('/', (req, res) => {
    ...
    <body>
        ...
        <button id="start-btn" style="margin-top:20px; padding: 10px;" onclick="socket.emit('start_game')">START GAME</button>
        ...
        <script>
            ...
            socket.on('update', (data) => {
                ...
                document.getElementById('start-btn').style.display = (data.gameStage === 'LOBBY' && data.isHost) ? 'inline-block' : 'none';
...
io.on('connection', (socket) => {
    ...
    socket.on('start_game', () => {
        ...
        playerOrder.forEach(id => players[id].chips = STARTING_CHIPS);
        startNewHand();
    });
